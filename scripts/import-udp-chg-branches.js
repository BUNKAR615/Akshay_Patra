/**
 * One-off: import UDP (Udaipur) & CHG (Chittorgarh) small-branch employees.
 *
 ...
 * EMPLOYEE password = empCode. Run: node scripts/import-udp-chg-branches.js
 */
...
const FILE = "C:/Users/Dinesh/Downloads/Required Data for UDP AND CHG Branch.xlsx";
    ...
    if (staleUsers.length > 0) {
      await tx.archivedEmployee.createMany({
        data: staleUsers.map((u) => ({
          ...
          archivedBy: "script:import-udp-chg", originalUserId: u.id,
        })),
      });
    }
...
async function main() {
  ...
  for (const { label, rows } of groups.values()) {
    const branch = allBranches.find((b) => b.name.trim().toLowerCase() === label.trim().toLowerCase());
    if (!branch) { errors.push(`No branch found for location "${label}" — group skipped`); continue; }
    const res = await replaceBranch(branch, rows);
    results.push({ branch: branch.name.trim(), ...res });
  }
