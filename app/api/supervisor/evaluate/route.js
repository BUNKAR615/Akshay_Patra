...
export const POST = withRole(["SUPERVISOR"], async (request, { user }) => {
    try {
        ...
        // Guard: employee exists and is in the supervisor's department
        const employee = await prisma.user.findUnique({ where: { id: data.employeeId }, select: { id: true, departmentId: true } });
        if (!employee) return notFound("Employee not found");
        if (employee.departmentId !== supervisor.departmentId) {
            return fail("You can only evaluate employees in your own department", 403);
        }

        // Guard: employee must be in Stage 1 shortlist
        const shortlistEntry = await prisma.shortlistStage1.findFirst({
            where: { userId: data.employeeId, quarterId: activeQuarter.id, departmentId: supervisor.departmentId },
        });
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            const shortlistCount = await tx.shortlistStage1.count({ where: { departmentId: supervisor.departmentId, quarterId: activeQuarter.id } });
            const evaluatedCount = await tx.supervisorEvaluation.count({ where: { supervisorId: user.userId, quarterId: activeQuarter.id } });

            let stage2Created = false;
            if (evaluatedCount >= shortlistCount) {
                // ── Small department rule: dynamic Stage 2 limit ──
                const deptLimits = await getDepartmentSize(supervisor.departmentId);
                const findArgs = { where: { supervisorId: user.userId, quarterId: activeQuarter.id }, orderBy: { stage2CombinedScore: "desc" } };
                ...
                await tx.shortlistStage2.deleteMany({ where: { departmentId: supervisor.departmentId, quarterId: activeQuarter.id } });
                ...
                for (let i = 0; i < allEvals.length; i++) {
                    ...
                    await tx.shortlistStage2.create({
                        data: { userId: ev.employeeId, quarterId: activeQuarter.id, departmentId: supervisor.departmentId, selfScore: sa?.normalizedScore || 0, supervisorScore: ev.supervisorNormalized, combinedScore: ev.stage2CombinedScore, rank: i + 1 },
                    });
                }
                stage2Created = true;
                ...
                if (deptLimits.caseNumber > 0) {
                    logSmallDepartmentRule({
                        userId: user.userId, departmentId: supervisor.departmentId,
        ...
        const response = {
            ...
            evaluation: { id: result.evaluation.id, employeeId: data.employeeId, supervisorScore: supervisorNormalized, combinedScore: combined, submittedAt: result.evaluation.submittedAt },
        ...
        if (result.stage2Created) {
            ...
            response.stage2Shortlist = { message: "All done! Top 5 auto-selected for Stage 2.", shortlist: stage2 };
