...
const MAX_HR_PER_BRANCH = 3;
const MAX_COMMITTEE_MEMBERS = 3;

/**
 * Rule A — one active role at a time.
 *
 * Rejects assigning `userId` to `targetRole` when the user already holds a
 * *different* BM/CM/HR/Committee assignment. Holding the same role on another
 * branch (CM/HR/Committee are legitimately multi-branch) is allowed.
 *
 * @param {string} userId
...
export async function assertSingleActiveRole(userId, targetRole) {
    ...
    const [bm, cm, hr, committee] = await Promise.all([
        ...
        prisma.committeeBranchAssignment.findFirst({ where: { memberUserId: userId }, select: { id: true } }),
    ]);
    ...
    if (hr) held.push("HR");
    if (committee) held.push("COMMITTEE");
...
/**
 * Committee eligibility — only role-holders may join the committee.
 *
 * A "normal employee" (User.role === EMPLOYEE with no evaluator/department-role
 * mapping) is NOT eligible. Any other role-holder — Supervisor, HOD, Branch
 * Manager, Cluster Manager, HR, Admin, or an existing Committee member — is.
 *
 * @param {string} userId
 * @returns {Promise<{ok: true} | {ok: false, message: string}>}
 */
export async function assertCommitteeEligible(userId) {
    ...
    const user = await prisma.user.findUnique({
        where: { id: userId },
        select: { role: true, _count: { select: { departmentRoles: true } } },
    });
    if (!user) return { ok: false, message: "User not found" };

    const isNormalEmployee = user.role === "EMPLOYEE" && user._count.departmentRoles === 0;
    if (isNormalEmployee) {
        return {
            ok: false,
            message: "Only role-holders (Supervisor, HOD, Branch Manager, Cluster Manager, HR, Admin, etc.) can be nominated to the committee. Normal employees are not eligible.",
        };
    }
    return { ok: true };
}
