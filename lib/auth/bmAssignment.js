 ...
 *
 * Two cases, mirroring the HR / CM / Committee assign routes so a Branch
 * Manager who is ALSO an employee keeps one home branch and dual-login (spec
 * rules 1, 3 & 5):
 *
 *   (1) EXISTING EMPLOYEE (has a departmentId) → PRESERVE their employee
 *       identity. departmentId / collarType are left UNTOUCHED so they remain
 *       in their original branch's employee list, and DUAL-LOGIN is configured:
 *         - password    = empCode       → EMPLOYEE dashboard (their home branch)
 *         - passwordHod = Firstname_##  → BRANCH MANAGER dashboard (this branch)
 *       Their original branch (department.branchId) never changes; the managed
 *       branch is carried by User.branchId + the BranchManagerAssignment row.
 *
 *   (2) PURE STAFF (no department) → original detach-on-promote: the staff
 *       formula is the PRIMARY password and the employee anchors (departmentId /
 *       passwordHod / collarType) stay null.
 *
...
export async function applyBmAssignment(tx, { userId, branchId, assignedBy, passwordHash }) {
    ...
    // Read the prior identity BEFORE writing, so we can decide between the
    // preserve-employee-identity and pure-staff branches, and so the caller can
    // still update the legacy department cache when the dept is in this branch.
    const priorUser = await tx.user.findUnique({
        where: { id: userId },
        select: { departmentId: true, empCode: true, name: true },
    });
    const priorDepartmentId = priorUser?.departmentId || null;

    let userData;
    if (priorDepartmentId) {
        // (1) EXISTING EMPLOYEE → preserve identity + dual-login. The staff
        // formula ("Firstname_##", already hashed in `passwordHash` when the
        // caller supplied one) becomes the SECONDARY password that opens the BM
        // dashboard; the PRIMARY password is reset to the empCode so the
        // employee login always keeps working — even if their password drifted.
        // departmentId / collarType are deliberately left untouched.
        const staffHash = passwordHash
            || await bcrypt.hash(
                defaultPasswordFor({ role: "BRANCH_MANAGER", empCode: priorUser.empCode, name: priorUser.name }),
                SALT_ROUNDS,
            );
        userData = {
            role: "BRANCH_MANAGER",
            branchId,
            passwordHod: staffHash,
            ...(priorUser.empCode ? { password: await bcrypt.hash(String(priorUser.empCode), SALT_ROUNDS) } : {}),
        };
    } else {
        // (2) PURE STAFF (no department) → detach. branchId is set to the new
        // branch (BMs are 1:1, so branchId is canonical for this role). The
        // employee anchors stay null so the user appears in no branch's
        // employee/HOD roster and a bulk re-upload cannot silently demote them.
        // When the caller supplied a passwordHash, also reset the password — a
        // pure-staff BM assignment writes the staff-format password
        // ("Firstname_##") as the primary so admins know the credentials.
        userData = {
            role: "BRANCH_MANAGER",
            branchId,
            departmentId: null,
