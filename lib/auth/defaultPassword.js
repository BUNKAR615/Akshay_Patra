...
/**
 * HOD secondary (role) password — `${Firstname}_${last 2 digits of empCode}`.
 *
 * HOD users have two passwords:
 *   - primary (User.password)        = empCode → routes to employee dashboard
 *   - secondary (User.passwordHod)   = Firstname_## → routes to HOD dashboard
 *
 * This helper produces the secondary form regardless of role, so callers
 * creating/updating HOD rows can hash and store it without re-implementing the
 * Firstname_## logic. Falls back to empCode if name/code lack the needed bits.
 */
export function defaultHodSecondaryPasswordFor({ empCode, name }) {
    const code = String(empCode || "").trim();
    if (!code) return "";
    const fn = firstName(name);
    const tail = lastTwoDigits(code);
