...
import { isTransientDbError } from "./http";
 ...
 *
 * Logs the error with a route label, then returns:
 *   - 503 (retryable) for transient DB connection errors — Neon cold start /
 *     pool wake-up — so clients retry gracefully instead of dead-ending.
 *   - 500 for genuine failures.
 *
 * @param {unknown} err   - the caught error
 * @param {string}  label - short route identifier for the log line
 */
export function handleApiError(err, label = "API") {
