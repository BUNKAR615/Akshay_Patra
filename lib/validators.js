...
export const hrEvaluateSchema = z.object({
    ...
    workingHours: z.number().min(0, "Working hours must be >= 0"),
    referenceSheetUrl: z
        .union([
            z.string().length(0),
            z.string().url("Reference sheet must be a valid URL"),
            z.string().regex(HR_REF_LOCAL_PATH_RE, "Invalid uploaded reference path"),
        ])
        .optional(),
